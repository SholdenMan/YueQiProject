...
#define FOREGROUND_STAR_IMAGE_NAME @"icon-zunyy-p"
#define BACKGROUND_STAR_IMAGE_NAME @"icon-zunyy-n"
#define DEFALUT_STAR_NUMBER 5
#define ANIMATION_TIME_INTERVAL 0.2
@interface NewStarView ()
...
- (UIView *)createStarViewWithImage:(NSString *)imageName {
    ...
    for (NSInteger i = 0; i < self.numberOfStars; i ++)
    {
        ...
        imageView.frame = CGRectMake(i * self.bounds.size.width / self.numberOfStars, 0, self.bounds.size.width / 8, self.bounds.size.height);
...
- (void)layoutSubviews {
    ...
    __weak NewStarView *weakSelf = self;
