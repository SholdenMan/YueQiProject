...
//        self.imageList = [NSArray arrayWithArray:imgArr];
        ...
        if ([direction isEqualToString:@"up"]) {
            ...
            //   button 从屏幕左侧出现
            self.frame = CGRectMake(0, btn.origin.y+btn.size.height, btn.size.width * 2 , *height);
        }
        table.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, *height);
...
-(void)hideDropDown:(UIButton *)b {
    ...
    //  隐藏的时候table的宽度
    table.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 0);
    [UIView commitAnimations];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self.btnSender addSubview:imgView];
    [self myDelegate];
}
